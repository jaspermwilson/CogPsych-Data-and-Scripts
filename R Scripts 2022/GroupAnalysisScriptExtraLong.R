...
Immediate_list <- list.files('500 ms (extra-long) duration/Raw Data (Immediate and Delayed)', pattern="[A-Z0-9]+(_[SMXLk0-9.]+ConstantTotalFinal[^\r\n]+)")
Delayed_list <- list.files('500 ms (extra-long) duration/Raw Data (Immediate and Delayed)', pattern="[A-Z0-9]+(_DelayedExp[^\r\n]+)")
...
for (initial_file in Immediate_list){
  participant_initials <- str_extract(initial_file, "[a-zA-Z0-9]+(?=_[SXMLk0-9.]+ConstantTotalFinal[^\r\n]+)")
  ...
  if(!is.null(initial_file) && !is.null(delayed_file) && !identical(delayed_file, character(0))){
    #Presented_Prime_count <- rbind(Presented_Prime_count, single_csv("500 ms (extra-long) duration/Raw Data (Immediate and Delayed)/","500 ms (extra-long) duration/Updated Processing/",initial_file, delayed_file, participant_initials))
    Correct_RT_Values <- rbind(Correct_RT_Values, single_csv("500 ms (extra-long) duration/Raw Data (Immediate and Delayed)/","500 ms (extra-long) duration/Updated Processing/",initial_file, delayed_file, participant_initials))
  }
