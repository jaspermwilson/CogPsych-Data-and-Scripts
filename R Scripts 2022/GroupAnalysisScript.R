...
Immediate_list <- list.files('48 ms (short) duration/Raw Data (Immediate and Delayed)', pattern="[A-Z0-9]+(_[SMLk0-9.]+ConstantTotalFinal[^\r\n]+)")
Delayed_list <- list.files('48 ms (short) duration/Raw Data (Immediate and Delayed)', pattern="[A-Z0-9]+(_DelayedExp[^\r\n]+)")


single_csv <- function(file_path_input, file_path_output, initial_file, delayed_file, initials, countDF){
  full_word_list = read.csv("FINAL-fullwordlist.csv")
  raw_immediate = read.csv(paste0(file_path_input, initial_file))
  delayed = read.csv(paste0(file_path_input, delayed_file))
...
#Presented_Prime_count <- data.frame();
Correct_RT_Values <- data.frame();
for (initial_file in Immediate_list){
  participant_initials <- str_extract(initial_file, "[A-Z0-9]+[A-Z0-9]+(?=_[SMLk0-9.]+ConstantTotalFinal[^\r\n]+)")
  delayed_file_bool <- str_detect(Delayed_list, paste0(participant_initials, "(_DelayedExp[^\r\n]+)"), negate = FALSE)
  delayed_file <- Delayed_list[delayed_file_bool]
  if(!is.null(initial_file) && !is.null(delayed_file) && !identical(delayed_file, character(0))){
    #Presented_Prime_count <- rbind(Presented_Prime_count, single_csv("48 ms (short) duration/Raw Data (Immediate and Delayed)/","48 ms (short) duration/Updated Processing/",initial_file, delayed_file, participant_initials))
    Correct_RT_Values <- rbind(Correct_RT_Values, single_csv("48 ms (short) duration/Raw Data (Immediate and Delayed)/","48 ms (short) duration/Updated Processing/",initial_file, delayed_file, participant_initials))
  }
  #write.csv(Presented_Prime_count, "Counts_and_proportion_for_presented_primes.csv")
  write.csv(Correct_RT_Values, "Prime_RT.csv")
